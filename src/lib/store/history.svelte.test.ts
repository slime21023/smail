...
describe('HistoryStore', () => {
	...
	it('undoes to the previous snapshot and redoes forward', () => {
		const history = new HistoryStore();
		history.capture(snap('a'));
		history.capture(snap('b'));
		history.capture(snap('c'));
	...
	it('drops consecutive identical snapshots (undo re-capture)', () => {
		const history = new HistoryStore();
		history.capture(snap('a'));
		history.capture(snap('b'));
		history.undo();
		// Restoring 'a' has the same full snapshot — it must not clear redo.
		history.capture(snap('a'));
	...
	it('returned snapshots are isolated clones', () => {
		...
		history.capture(snap('x'));
		expect(history.undo()?.settings.preheader).toBe('a');
	});

	it('keeps metadata-only changes as distinct history entries', () => {
		const history = new HistoryStore();
		const first = snap('same');
		const renamed = snap('same');
		renamed.settings.templateName = 'Renamed';
		history.capture(first);
		history.capture(renamed);
		expect(history.undo()?.settings.templateName).toBe(first.settings.templateName);
	});
});
